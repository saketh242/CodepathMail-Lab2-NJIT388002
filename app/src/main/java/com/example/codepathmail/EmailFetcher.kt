...
class EmailFetcher {
    companion object {
        ...
        const val title = "Welcome to Kotlin!"
        const val summary = "Welcome to the Android Kotlin Course! We're excited to have you join us and learn how to develop Android apps using Kotlin. Here are some tips to get started."
        fun getEmails(): MutableList<Email> {
            var emails : MutableList<Email> = ArrayList()
            for (i in 0..9) {
                val email = Email(senders[i], title, summary, R.drawable.android,false)
                emails.add(email)
            }
            return emails
        }
